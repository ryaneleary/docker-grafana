...
if [ "${INFLUXDB_SRV}" = "**ChangeMe**" ]; then
    ...
    export INFLUXDB_HOST=$(dig ${INFLUXDB_SRV} SRV | grep SRV | grep -v "^;" | head -n 1 | awk '{print $8}' | rev | cut -c 2- | rev)
    export INFLUXDB_PORT=$(dig ${INFLUXDB_SRV} SRV | grep SRV | grep -v "^;" | head -n 1 | awk '{print $7}')
fi
